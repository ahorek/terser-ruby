...
def expect_to_have_inline_source_map(minified, original)
  ...
  _, map = Terser.compile_with_map(minified, options)
...
describe "Terser" do
  ...
  it "generates source maps" do
    ...
    minified, map = Terser.new.compile_with_map(source)
  ...
  it "generates source maps with the correct meta-data" do
    _, json = Terser.compile_with_map(
      source,
      :source_map => {
        :filename => "ahoy.js",
  ...
  it "proceses an input source map" do
    ...
    minified1, map1 = Terser.compile_with_map(
      source,
      :source_map => {
        :filename => "ahoy.js",
        :root => "http://localhost/"
    ...
    _, map2 = Terser.compile_with_map(
      source,
      :source_map => {
  ...
  it "handles empty string as input map sourceRoot" do
    _, map1 = Terser.compile_with_map(
      source,
      :source_map => {
        :filename => "ahoy.js",
        :root => ""
  ...
  it "appends source map url to minified JS" do
    minified, = Terser.compile_with_map(
      source,
      :source_map => {
        :filename => "ahoy.js",
        :output_filename => "ahoy.min.js",
  ...
  describe "inline source map parsing" do
    ...
    it "only parses source maps at end of file" do
      minified = "#{code}\n//#{source_mapping_url}\nhello();"
      _, map = Terser.compile_with_map(minified)
